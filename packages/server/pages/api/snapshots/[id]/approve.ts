...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  await prisma.$transaction([
    ...
    prisma.baseline.upsert({
      where: {
        projectId_branch_name_viewport: {
          projectId: snapshot.build.projectId,
          branch: snapshot.build.baseBranch,
          name: snapshot.name,
      ...
      update: { imageKey: snapshot.imageKey, snapshotId: snapshot.id },
    }),
  ]);
  ...
  await syncCommitStatusIfConfigured(build.project, {
    sha: build.commitSha,
    state: status === "NEEDS_REVIEW" ? "failure" : "success",
    description:
      status === "APPROVED" ? "All visual changes approved" : "Visual changes reviewed",
